...
if($output_gene_conf_list_with_info == 1) {
    $outfile =~ s/.txt$//i;
    $outfile =~ s/.html$//i;
    $outfile =~ s/.htm$//i;
    $outfile =~ s/.text$//i;
    my $gene_conf_outfile = $outfile . "-gene_conf_list_with_info.txt";
    open(GENECONFLISTOUTFILE, ">$gene_conf_outfile");
    print GENECONFLISTOUTFILE "$headerstuff\n";
    for(my $i=$start; $i<$num_conds; $i++) {
	print GENECONFLISTOUTFILE "Condition $i Upregulated Genes\n-----------\n";
	foreach my $id (sort {$gene_confidences_up{$b}[$i]<=>$gene_confidences_up{$a}[$i]} keys %gene_confidences_up) {
	    my $xx = $gene_confidences_up{$id}[$i];
	    my $info = $id2info{$id};
	    print GENECONFLISTOUTFILE "$id\t$xx\t$info\n";
	}
	print GENECONFLISTOUTFILE "\nCondition $i Downregulated Genes\n-----------\n";
	foreach my $id (sort {$gene_confidences_down{$b}[$i]<=>$gene_confidences_down{$a}[$i]} keys %gene_confidences_down) {
	    my $xx = $gene_confidences_down{$id}[$i];
...
#			    print "RATIO = $val\n";
		...
		if($stat == 0 && !($tstat_tuning_param =~ /\S/)) {
			...
			$val = $val_vect[$i];
			if($val ne "NA") {
			    if($val>=$center) {  # it's an upregulated one
				if($val>$max_vect[$i][$cond] || ($max_vect[$i][$cond] == $center)) { # in this case it is more extreme than any non-permuted
				    $temp_up_vect[$i][$num_bins]++;  # so increment the top bin
				}
				else {
			    ...
			    if($val<=$center) { # downregulated case
				if($val<$min_vect[$i][$cond] || (-$min_vect[$i][$cond] == $center)) {
...
# unpermuted data.
	...
	for(my $id=0; $id<$num_ids; $id++) {
		...
		for(my $i=0; $i<$num_range_values; $i++) {
		    ...
		    if($value ne "NA") {		
			if($value>=$center) {
			    if($max_vect[$i][$cond] == $center) {
				$dist_up_vect[$i][$cond][$num_bins]++;  # increment the top bin
			...
			if($value<=$center) {
			    if(-$min_vect[$i][$cond] == $center) {
				$dist_down_vect[$i][$cond][$num_bins]++;
				$dist_up_vect[$i][$cond][0]++;				
			    }
			    else {
				my $bin=int($num_bins * (-$value - $center) / (-$min_vect[$i][$cond] - $center));
	...
	for(my $i=1; $i<$num_conds; $i++) {
		...
		for(my $p=0; $p<$n; $p++) {
			...
			$perm_array[$i][$p][$j+$num_reps[0]] = 1-$temp_array[$p][$j];
                        if($number_subsets >= $num_perms+25 && $p==0) { # this makes sure the trivial perm is one of them
			    $perm_array[$i][$p][$j] = 1;
